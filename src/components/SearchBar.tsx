...
export default function SearchBar({ initialQuery }: { initialQuery: string }) {
  ...
  // --- DEFINITIONS MOVED INSIDE THE COMPONENT ---

  // Debounced search logic (uses useCallback)
    ...
    const timeoutId = setTimeout(async () => {
      if (searchTerm.trim().length > 2) {
        try {
          ...
          setSuggestions(data.results.slice(0, 5).map(m => ({ id: m.id, title: m.title })));
        } catch (error) {
    ...
    <div className="relative mb-8 max-w-2xl mx-auto">
      <form onSubmit={handleSubmit} className="relative">
        <input
          type="search"
          value={query}
          onChange={handleInputChange} // This now correctly references the function above
